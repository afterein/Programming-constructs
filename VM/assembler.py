...
def parse_command(line, arg1_type=None, arg2_type=None):
    ...
    if arg1_type is None:
        ...
        Check(arg1 is not None, "wrong argument " + line[1])
    elif arg1_type == "line":
        try:
            arg1 = (int(line[1]) - 1) * COMMAND_LEN + FIRST_COMMAND_ADDR
        except ValueError:
            Check(False, "wrong argument " + line[1])
    arg2 = 0
    if arg2_type is None:
        pass
    elif arg2_type == "reg":
        arg2 = registers.get(line[2])
        Check(arg2 is not None, "wrong argument " + line[2])
    elif arg2_type == "int":
        try:
            arg2 = int(line[2])
        except ValueError:
            Check(False, "wrong argument " + line[2])
    return [arg1, arg2]
...
if __name__ == '__main__':
    if len(sys.argv) not in [2, 3]:
        ...
        exit(-1)
    output_path = str(sys.argv[1]) + ".btc"
    if len(sys.argv) == 3:
