...
class BinaryFile:
    ...
    def write(self, address, data):
        Check(address < self.size, "Attempt to write at address {0} when program size is {1}".format(address, self.size))
        self.buff[address] = data

    def read(self, address):
        Check(address < self.size, "Attempt to read at address {0} when program size is {1}".format(address, self.size))
...
class VirtualMachine:
    ...
    def movn(self, where, what):
        Check(where not in self.private_reg, "Attempt to write {1} to protected memory {0}".format(where, what))
    ...
    def jmp(self, where):
        Check(where > EDX, "Attempt to execute data")
    ...
    def pop(self, where):
        Check(self.at(ESP) >= self.at(EBP), "Poping from empty stack")
        self.memory.write(ESP, self.at(ESP) - 1)
    ...
    def ret(self):
        self.memory.write(ESP, self.at(EBP))
        self.memory.write(EBP, self.at(self.at(EBP)))
        self.memory.write(EIP, self.at(self.at(ESP) + 1))
